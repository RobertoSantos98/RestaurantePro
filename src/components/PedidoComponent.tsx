    ...
    return (
        <div className={`bg-white relative p-4 rounded-2xl shadow mb-4 w-auto sm:w-80 md:w-96 lg:w-1/4 ${pedido.status !== "PENDENTE" ? "" : "border-2 border-red-500 shadow-md shadow-red-500"} flex flex-col gap-2 hover:shadow-2xl transition-all`}>
            ...
            <div className="flex justify-between">
                ...
                {/* <p className="text-gray-800 text-3xl font-bold">R$ {pedido.valor.toFixed(2)}</p> */}
            </div>
            ...
            <div className="flex flex-wrap gap-2 justify-center">
                ...
                {pedido.status !== "PENDENTE" ? "" : <button className="bg-blue-500 text-white px-4 py-2 rounded font-bold hover:bg-blue-600 w-full max-w-80">Imprimir</button>}
            </div>
        </div>
    );
}